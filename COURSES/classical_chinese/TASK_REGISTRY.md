...
| `TASK-CLCH-GOV-003` | governance | next | define source-authority and copyright boundary rules before material extraction | source-authority contract, copyright boundary, locator and excerpt rules, review preconditions | lesson plans, full-text copying, material promotion without review |
...
3. `TASK-CLCH-GOV-003` must complete before `TASK-CLCH-MAT-001` may begin.
4. `TASK-CLCH-MAT-*` may structure source-derived course material below lesson level, but may not generate formal lesson artifacts.
5. `TASK-CLCH-DES-*` may shape teaching architecture only after the `16-session core material matrix` exists.
6. `TASK-CLCH-AI-*` belongs to pedagogy and workflow method, never to disciplinary content substitution.
7. `TASK-CLCH-ASSESS-*` begins only after content and design boundaries are stable enough for assignment suitability review.
