...
1. `TASK-LPW-GOV-001` does not create a remote repository and does not push.
2. GitHub creation and authentication require a later task.
3. Never use a GitHub account password for automation.
4. Prefer `gh auth login`; if `gh` is unavailable, use GitHub's web UI and a token.
